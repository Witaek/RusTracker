//this fonction return the airbone position when given an odd and an even message
...
fn j_calcul(cpr_lat_even : f64, cpr_lat_odd : f64) -> f64 {
    return (((59. * cpr_lat_even - 60. * cpr_lat_odd) as f64) + 0.5).floor();
}
...
fn coor(even_data: u64, odd_data: u64) -> (f64,f64) {
    ...
    //cpr conversion
    let cpr_lat_even = (u32::from_str_radix(even_data_tab[6].as_str(), 2).unwrap() as f64) / 131072.;//ma value of (2^17)
    let cpr_lat_odd = (u32::from_str_radix(odd_data_tab[6].as_str(), 2).unwrap() as f64) / 131072.;
    let cpr_lon_even = (u32::from_str_radix(even_data_tab[7].as_str(), 2).unwrap() as f64) / 131072.;
    let cpr_lon_odd = (u32::from_str_radix(odd_data_tab[7].as_str(), 2).unwrap() as f64) / 131072.;

    //index j calcul
    let j = j_calcul(cpr_lat_even, cpr_lat_odd);

    let mut lat_even = d_lat_even * ((j%60.0) + cpr_lat_even);
    let mut lat_odd = d_lat_odd * ((j%60.0) + cpr_lat_odd);
    ...
    //longitude zone size
    let n_even = nl.max(1.);
    let n_odd = nl_calcul(lat-1.0).max(1.);
